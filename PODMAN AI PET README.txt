# Podman + Ollama + Open WebUi Setup Guide for Linux Mint, Ubuntu
...
#=======================================================================
# STARTING OLLAMA AND Open WebUi - HOSTING AND TALKING TO YOUR AI PET
#=======================================================================
...
# Next, ensure the internal network is running. This allows Ollama and Open WebUi to communicate (important)
podman network create llm-network
...
#=================================================================
# OPEN WEBUI CONFIGURATION - YOUR COMMUNICATOR WITH YOUR AI PET
#=================================================================
...
#==================================================
# When you are done interacting with your LLMs run the below two lines of code to stop both Ollama and Open WebUi and remove their hosted containers:
...
#==================================================
# Whenever you want to restart your containers, simply run the script from 'STARTING OLLAMA AND Open WebUi' above.

# If you run into permission issues, check troubleshooting.

# You may also need to recreate llm-network, which is the internal means by which Ollama talks to Open WebUi:
podman network create llm-network
...
# Open WebUi data: /mnt/ssd/podman/webui-data/
...
# Test Ollama is accessible from Open WebUi
podman exec open-webui curl http://ollama:11434/api/tags
...
podman pull docker.io/ollama/ollama:latest
podman pull docker.io/mintplexlabs/anythingllm:latest FIND
...
# CONTAINER INSPECTION. 'podman ps -a' is actually quite useful though.
#--------------------
# View logs
podman logs ollama
...
# Inspect volume
podman volume inspect Open WebUi_storage

# Show volume size
podman volume inspect Open WebUi_storage | grep -i size

# Backup volume (while container stopped)
podman run --rm -v Open WebUi_storage:/source:ro -v $(pwd):/backup alpine tar czf /backup/Open WebUi-backup.tar.gz -C /source .
